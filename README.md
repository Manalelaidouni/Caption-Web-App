...
**TL;DR** : It contains the fully customizable DL pipeline that ensures  reproducibility and modularity, it covers designing an end-to-end training pipeline including language and vision data processing, model architecture development implemented in Pytorch, training, transfer learning, evaluation, as well as monitoring and logging and also inference.


Additionally,  developed and API deployment process by building a production environment within an AWS EC2 instance with containerization to ensure the application works seamlessly across different environments, build with Flask, Gunicorn, Nginx, Docker, and Docker-compose.
...
Create and activate a virtual environment:
 you can name it what you want, I called mine streamlit_env
...
To train the model using gpu install torch gpu version with cuda:
  ```
...
Train for specific epochs from scratch:
  ```
...
To setup LR Finder for training using the Shell Script, this will generate a plot like <a href="#LRFinder_img">this</a> one and  save to `plots` folder :
...
- **Command line arguments**: Alternatively, you can use command line arguments to customize training, here is the optional arguments:
...
> I have implemented all of the NLP preprocessing within the `Preprocessor` class, you can find it <a herf="https://github.com/Manalelaidouni/CaptionCode/blob/fdd5f0cd5d5004bb7bcb5be8dedf3dfce63a728f/preprocess_nlp.py#L23">here</a>.
>
...
We construct a vocabulary to represent each token with a unique number to be ready to be processed by a neural network. We start by excluding tokens with low frequencies, we then create a dictionary that maps the tokens to a unique index. We reserve the first 4 indices for special tokens  `<sos>` to indicate the beginning of a caption, `<end>` to signify the end of a caption, `<pad>` for identifying padding tokens (which we will explain later- **Insert Link to pad**) and `<unk>` to represent out-of-vocabulary words encountered during inference. It’s important to note that we built the vocabulary using only training data to avoid any data leakage.
...
Furthermore, it's crucial to adjust the image size to match the expected size from the used neural network, for instance, `ResNet` expects an image size of `224x224` dimensions, while `Inception-V3` expects an image size of `256x256` dimensions.

After resizing and converting the batch of images into Torch tensors, we proceed to normalize the images. This is where we normalize the values of each RGB channel in the image using the provided mean and standard deviation (std) values. We utilize the statistics of the dataset that the pre-trained CNN model was trained on, and since most pretrained models are trained on ImageNet, we use the mean and std values specific to the ImageNet dataset  like the following `transforms.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))`.

<br>

> When selecting data augmentation techniques, we consider the type of images we want the model learn from best. Consequently we avoid using data augmentation techniques that are not suitable for the type of data we have, e.g, flipping images of digits like 9 to be 6 will lead the network to learn wrong incorrect associations. In our case, for image captioning, we don’t crop images since captions describe the entire image and cropping could remove objects mentioned in the captions. Therefore we just opted for resizing and normalizing the images.
...
To facilitate the loading of the preprocessed data into our network, Pytorch uses `DataLoader` which  offers many features such as batching, multi-processing , data augmentation, etc. While batching image tensors is relatively straightforward (stacking images of the same shape), the case is less straightforward with text data, because unlike images text sequences can have varying sizes. To accommodate for this, we implement the `collate()` function, which pads captions of different sizes to a uniform size. 
(Link pad to this location)
...
Meanwhile  `DataLoader` returns a batch of inputs batch of images of dimensions  `[batch_size, depth, Height, Width]` paired with their corresponding captions of size `[max_seq_len, batch_size]` that were padded with zeros to the length of the longest caption `max_seq_len` using the previously mentioned `collate()` function.
...
> tl;dr : I implemented  an Encoder decoder architecture with a pre-trained ResNet CNN  as an encoder to extract visual features from the images coupled with a  Long Short-Term Memory (LSTM) network as a decoder to decode these features to generate a caption, the LSTM is coupled with  Bahdanau  attention mechanism, which allows the decoder to focus on parts of the image that are relevant to generating the next token in the description sequence.
>
...
We omitted last two layers and replaced it with an Adaptive Average Pooling layer, which adapts the  generated feature map to a specific size, resulting in final  feature map of dimensions `[bs, encoder_size, encoder_size, depth]` [16 , 18, 18, 2048]. 
...
This is achieved by computing an attention score for each visual feature and  these scores are then used to form a weighted sum of the feature map known as the context vector. There are different types of attentions,  we implemented the Bahdanau et al.’s attention from the [ show attend and tell paper](https://arxiv.org/abs/1409.0473). During training, the context vector is then incorporated with the current input to the LSTM. 
...
<p align="center"> <i><font color="grey">The t-SNE plot visualizes the two-dimensional projections of Glove pre-trained 50-dimensional embeddings, we produced these projections  by applying PCA decomposition algorithm. Additionally, the color coding represents the clusters the words belong to, we determined these clusters by using the K-means clustering algorithm.</font> </i></p>


<br>

We can clearly see that words with similar semantic meanings are plotted closely to each other,  for instance,  months and years in purple clustered towards center right, while the numbers are colored in yellow, while words related to sports are in purple (cup, games, season, championship, club, etc). Words that are related to government and politics. are colored in burgundy (foreign, ministry, commission, agency, u.n, etc) while those related to war are colored in pink (military, force, attacks, war, soldiers, etc). Family and relationships (son, wife, father, man, women, children, family, he, her, she, etc) are clustered in the top right section shown in orange.
...
It's worth noting that I added a dropout layer with a rate of 0.1 on top of the lstm layer at each decoding time step to enhance the model's ability to generalize, the resulting output is then passed through a fully connected  layer to generate logits (unnormalized scores) for each token in the vocabulary, this process results in a tensor of dimensions `[batch_size, vocab_size]`.
...
At this stage, we do not apply the `softmax` activation function to the logits, because the `Cross Entropy loss` function provided by PyTorch internally implements the `log-softmax` operation, hence we keep the output as logits. This brings us to the loss function we used.
...
We use `Cross Entropy` loss function to calculate the error for a single batch at the end of each forward pass.  Its purpose is to quantify how close the model predictions are to the ground truth target. 
...
For the choice of the optimizer we used Adam optimizer with a `weight_decay` rate of 0.01, the learning rate we used is determined using the LR Range test and adjusted during training using LR  scheduler, we will address the details in this <a href="#LRFinder">section</a>. 
...
At this point, the codebase was set up to try out different ways of adjusting learning rates, prevent overfitting, and fine-tune various parts of the model, like the encoder and embeddings. While it was tempting to jump into experimenting right away, I decided to run some sanity checks. I started by working with a small model before scaling which didn't include complex features like attention and other advanced training techniques.
...
I customized the LR Finder code to accommodate to the input and label structures used in my model.
...
<p align="center"><i><font color="grey">
Once Cycle Policy cyclical training scheduler, The maximum learning rate was determined with the LR range finder.<a href="https://www.deepspeed.ai/tutorials/one-cycle/"> Source</a>.</font></i></p>



<br>

> The effectiveness of the  Once Cycle Policy lies in its ability to speed up convergence while preventing overfitting, thanks to the long lr warm up during the first half of the cycle makes the model quickly converge to a good solution, while during the second half of the cycle, the learning rate decreases, which helps the model fine-tune and reduce the risk of overfitting.


<br>

To avoid overfitting, we also implemented `Early Stopping` for  a patience of 10 epochs that is if the validation Bleu score does not improve for 10 consecutive epochs, the training process is stopped.  In other words, training halts when the model's performance stops improving on the hold out set.

<br>


The model was trained for 20 epochs on training data and then evaluated on validation data for each epoch. Furthermore, we saved the best performance achieved so far based on either the loss or the Bleu score. Specifically, we saved the best checkpoint with the lowest score if the tracked metric of choice is the loss, and we saved the best checkpoint with the highest score if the metric is Bleu score. 
...
However, since early stopping was already implemented and training was not halted before it means that there was a chance that the model was going to improve if we trained for longer epochs.  Unfortunately, this was not possible due to computational resource constraints.

That brings us to the Bleu score we used to evaluate the similarity between the model's generated caption and the truth label caption.

<br>

### Bleu score metric 

As mentioned above, we used the  `BLEU ` (Bilingual Evaluation Understudy) metric to quantify the similarity between the predicted captions and the reference ground truth captions. This metric is implemented using the `corpus_bleu` function available in the `nltk.translate.bleu_score` module of the NLTK library.
...
We created the production environment  inside an `AWS EC2 Linux instance`, specifically using a t3 micro instance that we connect to using SSH.

To enable users to perform inference using our model, we implemented a REST API using `Flask` framework to manage rendering of templates and routing  between the client and the Python app coupled with front end designed using HTML, CSS and added  Bootstrap elements footer to make code responsive. 

In the Flask part, when the client sends data using the `POST` method, in this case an uploaded image from the user, the `upload()` function is executed. This function performs essential security  measures on the passed data and then runs inference, ultimately retrieving the desired caption output.
...
However,  Nginx only handles static requests by serving static media files such as images or CSS files directly from the file system, and it can not handle dynamic requests like submitting the image for caption generation nor does it communicate directly with the Flask app. This is why we use  `Gunicorn` to act like a bridge to enable the communication between the flask app and the client.
...
> We containerized the entire process using `Docker` so that the web app is portable and can run consistently and smoothly across different environments without relying extensively on virtual environments.
...
For sanity check, we build the container for Flask first :
```
...
As mentioned above, Docker-compose allows the connection between the containers, here is the file:
...
After finishing all configuration, we can spins everything up locally as follow :
...
<details>
<summary>Replicating the Production Environment </summary>
...
Finally, we assigned a fixed IP address to the EC2 instance instead of generating a new IP address for the instance on relaunch, you can access the web app <a href="http://13.50.96.179">here 🌐</a>.
...
- Connect with me on [LinkedIn](https://www.linkedin.com/in/manal-e-440a7b292/).
